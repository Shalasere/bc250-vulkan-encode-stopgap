...
- **H.265/HEVC is a non-functional stub — TODO, tracked on `feature/hevc-h265`.** `encoder_h265.c` is advertised as a working `VAProfileHEVCMain` capability, but it is not one: its SPS/PPS/VPS are missing large numbers of mandatory HEVC syntax fields (a real decoder will very likely fail to parse the SPS at all), and its slice writer emits a single flag bit and stops — no slice type, no QP, no reference picture data, no picture content whatsoever. It does dispatch the real GPU compute pipeline, then discards the result entirely without ever serializing it. If an application selects HEVC (Sunshine's config has an `hevc_mode` setting, and many streaming clients prefer HEVC automatically for bandwidth), it will get silent, unusable garbage. **Do not enable HEVC anywhere pointed at this driver until this is built for real** — it is not a small fix: HEVC always uses CABAC (no CAVLC option), so essentially none of this project's H.264 entropy-coding work carries over; it's a comparable-scope effort to the H.264 correctness work, redone for a harder codec. Stick to H.264 (`encoder=vaapi` with the default codec, not `hevc_mode=1`) until this lands.
