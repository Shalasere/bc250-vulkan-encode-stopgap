...
## APU Specifications
- **Architecture**: Zen 2 CPU (8 cores / 16 threads) + semi-custom "Oberon"/Cyan Skillfish GPU (`gfx1013`) - not simply RDNA2: an RDNA1-generation memory subsystem (no Infinity Cache/System Level Cache) with RDNA2-generation ray-tracing BVH units grafted on. Corrected from an earlier flat "RDNA 2" label here.
...
1. **Physical VCN is Unusable & Permanently Fused**:
   ...
   - This driver (`bc250-vulkan-encode-stopgap`, formerly `bc250-encoding-decoding-fix`, formerly `bc250-vcn-driver`) provides the community's sole hardware-accelerated encoding solution by translating VA-API calls into Vulkan Compute shaders executed on the GPU's Compute Units.
